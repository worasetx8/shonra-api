...
async function migrateEnsureAllTables() {
  try {
    ...
    // Define all required tables (15 tables)
    ...
    // Create all tables (will skip if already exists due to CREATE TABLE IF NOT EXISTS)
    // This ensures all tables are created in one run, even if some already exist
    
    // 1. Create admin_users table (always create, IF NOT EXISTS will skip if exists)
    ...
    // 2. Create settings table (always create, IF NOT EXISTS will skip if exists)
    const settingsExists = existingTables.has('settings');
      await executeAndCheck(`
