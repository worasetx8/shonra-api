    ...
    try {
      result = await executeQuery(`
        SELECT 
          ...
          maintenance_mode, maintenance_bypass_token, version,
          ...
          google_verification_code, bing_verification_code,
          enable_ai_seo, gemini_api_key, ai_seo_language, canonical_url, robots_meta,
      ...
      if (error.code === 'ER_BAD_FIELD_ERROR') {
        ...
        try {
          result = await executeQuery(`
            ...
            FROM settings WHERE id = 1
          `);
          // If gemini_api_key column doesn't exist, add it as null
          if (result.success && result.data.length > 0) {
            result.data[0].gemini_api_key = null;
          }
        } catch (fallbackError) {
          ...
          result = await executeQuery(`
            ...
            FROM settings WHERE id = 1
          `);
          // Add null values for SEO fields
          if (result.success && result.data.length > 0) {
            result.data[0].gemini_api_key = null;
          }
        }
        ...
        if (error.code === 'ER_BAD_FIELD_ERROR') {
          try {
            ...
            if (newResult.success && newResult.data.length > 0) {
              newResult.data[0].gemini_api_key = null;
            }
          } catch (fallbackError) {
            // If even basic SEO columns don't exist, use minimal columns
            newResult = await executeQuery(`
